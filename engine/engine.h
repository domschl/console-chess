...
vector<Move> rawCaptureList() {
    ...
    // negamax / alpha-beta
    static int negamax(Board brd, int depth, vector<Move> &history, vector<Move> &principal, int col,
...
void miniGame() {
    ...
    for (int i=0; i<50; i++) {
        ...
        vector<Board::Move> ml(Board::searchBestMove(brd,5));
        if (ml.size()==0) {
