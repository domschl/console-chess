...
public:
  ...
  void addMove(vector<char *> &ml, char *pos, int of, int ofn, int f = 0,
    ...
    // wcout << ROW(of) << COL(of) << " " << ROW(ofn) << COL(ofn) << " " << val
    // << endl;
    if (val)
      ml.push_back(posn);
    else
          ...
          if (col == CW) {
            if (pos[POS_WKC] == 1) {
              if (pos[FOF(7, 5)] == 0 && pos[FOF(7, 6)] == 0 &&
                  !isThreatened(pos, 7, 4, CW) && !isThreatened(pos, 7, 5, 1)) {
                addMove(ml, pos, FOF(7, 4), FOF(7, 6), 0, FOF(7, 7), FOF(7, 5));
              }
            }
            if (pos[POS_WQC] == 1) {
              if (pos[FOF(7, 1)] == 0 && pos[FOF(7, 2)] == 0 &&
                  pos[FOF(7, 3)] == 0 && !isThreatened(pos, 7, 3, 1) &&
          ...
          if (col == CB) {
            ...
            if (pos[POS_BQC] == 1) {
              if (pos[FOF(0, 1)] == 0 && pos[FOF(0, 2)] == 0 &&
                  pos[FOF(0, 3)] == 0 && !isThreatened(pos, 0, 3, CB) &&
                  !isThreatened(pos, 0, 4, CB)) {
  ...
  int posScore(char *pos) { return *(int *)&pos[POS_SCORE]; }

  int alphabeta(char *pos, int a, int b, int col, bool bMax, int depth,
                int curdepth, int *pcurnodes, time_t maxtime, wstring &movstack,
                char **pnpos, vector<char *> *pmlo) {
    int bsc, sc, bv, v;
    char *bp = nullptr;
    int nr, silnr;
    bool bTo = false;
    ...
    if (bMax) {
      ...
      for (auto pi : ml) {
        ...
        if (!cacheRead(pi, &v, depth - curdepth)) {
          v = alphabeta(pi, a, b, CB, false, depth - 1, curdepth + 1, pcurnodes,
                        maxtime, movstack, nullptr, nullptr);
      ...
      for (auto pi : ml) {
        ...
        if (!cacheRead(pi, &v, depth - curdepth + 1)) {
          v = alphabeta(pi, a, b, CW, true, depth - 1, curdepth + 1, pcurnodes,
                        maxtime, movstack, nullptr, nullptr);
          if (v != SC_TIMEOUT_INVALID)
        ...
        if (!bTo) {
          ...
          *(int *)&pi[POS_SCORE] = sc;
          if (sc > bsc) {
            ...
            beststack = movstack;
            if (curdepth < 1)
              wcout << movstack << " Score: " << bsc << " @ " << curdepth
                    << endl;
          }
          if (v < bv)
            bv = v;
  ...
  int makeMove(char *pos, int col, std::map<string, int> &heuristic,
    ...
    for (depthi = 4; depthi <= depth; depthi++) {
      ...
      int b = SC_MAX;
      wstring buf = L"";
      sc = alphabeta(pos, a, b, col, true, depthi, 0, pcurnodes, maxtime, buf,
                     pnpos, &ml);
...
int main(int argc, char *arv[]) {
  ...
  Chess c(3000000); // no. of cache entries
  ...
  while (bOk) {
    ...
    if (npos != nullptr) {
      ...
      wcout << move << L" Nodes: " << curnodes << L", Nodes/sec: " << nps
            << L", time:" << dt << L"s" << endl;
      wcout << L" cache-hits:" << c.cachehit << L", cachemiss:" << c.cachemiss
            << L", cacheconfl:" << c.cacheclash << endl;
      col = col * (-1);
