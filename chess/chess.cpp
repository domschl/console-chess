...
#define POS_WKC                                                                \
    (POS_CTRL_OFFSET +                                                         \
     1)  // Flag is white can castle king-side: 0:no 1: yes 2: done
#define POS_WQC (POS_CTRL_OFFSET + 2)  // queen side:  0:no 1: yes 2: done
#define POS_BKC                                                                \
    (POS_CTRL_OFFSET + 3)  // Black costle king side:  0:no 1: yes 2: done
...
#define POS_NEXTCOL                                                            \
    (POS_CTRL_OFFSET + 12)  // 1: white to move, -1 black to move.
...
class Chess {
  public:
    ...
    Chess(int cachsize = 10000000) {
        ...
        char iF[] = {FIELD_WR, FIELD_WN, FIELD_WB, FIELD_WQ,
                     FIELD_WK, FIELD_WB, FIELD_WN, FIELD_WR};
        for (cx = 0; cx < 8; cx++) {
    ...
    bool isThreatened(char *pos, int cy, int cx, int col) {
        ...
        int d, of;
        of = FOF(cy, cx);
        if (pos[FOF(cy - col, cx + 1)] == FIELD_BP * col)
            return true;
        if (pos[FOF(cy - col, cx - 1)] == FIELD_BP * col)
            return true;
        for (d = 0; d < sizeof(no) / sizeof(no[0]); d++) {
            if (pos[of + no[d]] == FIELD_BN * col)
                return true;
        }
        for (d = 0; d < sizeof(bo) / sizeof(bo[0]); d++) {
            ...
            while (val) {
                of0 += bo[d];
                if (pos[of0] == FIELD_BB * col || pos[of0] == FIELD_BQ * col)
                    return true;
                if (pos[of0] != FIELD_EMPTY)
                    val = false;
            }
        }
        for (d = 0; d < sizeof(ro) / sizeof(ro[0]); d++) {
            ...
            while (val) {
                of0 += ro[d];
                if (pos[of0] == FIELD_BR * col || pos[of0] == FIELD_BQ * col)
                    return true;
                if (pos[of0] != FIELD_EMPTY)
                    val = false;
            }
        ...
        for (d = 0; d < sizeof(ko) / sizeof(ko[0]); d++) {
            if (pos[of + ko[d]] == FIELD_BK * col)
                return true;
        }
        return false;
    }

    void addMove(vector<char *> &ml, char *pos, int of, int ofn, int f = 0,
                 int of1 = 0, int ofn1 = 0) {
        char *posn = (char *)malloc(POS_SIZE * sizeof(char));
        memcpy((void *)posn, pos, POS_SIZE * sizeof(char));
        posn[POS_CAPT] = FIELD_EMPTY;
        if (posn[ofn] != FIELD_EMPTY)
            posn[POS_CAPT] = posn[ofn];
        posn[ofn] = pos[of];
        posn[of] = FIELD_EMPTY;
        posn[POS_LASTMV0] = of;
        ...
        if (f0 == FIELD_WR && of == FOF(7, 0))
            posn[POS_WQC] = 0;
        if (f0 == FIELD_WR && of == FOF(7, 7))
            posn[POS_WKC] = 0;
        if (f0 == FIELD_BR && of == FOF(0, 0))
            posn[POS_BQC] = 0;
        if (f0 == FIELD_BR && of == FOF(0, 7))
            posn[POS_BKC] = 0;
        bool val = true;
        posn[POS_WK_CHK] = 0;
        posn[POS_BK_CHK] = 0;
        bk = posn[POS_BK];
        if (isThreatened(posn, ROW(bk), COL(bk), CB))
            posn[POS_BK_CHK] = 1;
        wk = posn[POS_WK];
        if (isThreatened(posn, ROW(wk), COL(wk), CW))
            posn[POS_WK_CHK] = 1;
        if (f0 < 0) {
            if (posn[POS_BK_CHK])
                val = false;
        ...
        // wcout << ROW(of) << COL(of) << " " << ROW(ofn) << COL(ofn) << " " <<
        // val
        // << endl;
        if (val)
            ml.push_back(posn);
        ...
        for (cy = 0; cy < 8; cy++) {
            for (cx = 0; cx < 8; cx++) {
                ...
                if (fig == FIELD_WK) {
                    ...
                    if (col == CW) {
                        if (pos[POS_WKC] == 1) {
                            if (pos[FOF(7, 5)] == 0 && pos[FOF(7, 6)] == 0 &&
                                !isThreatened(pos, 7, 4, CW) &&
                                !isThreatened(pos, 7, 5, 1)) {
                                addMove(ml, pos, FOF(7, 4), FOF(7, 6), 0,
                                        FOF(7, 7), FOF(7, 5));
                            }
                        }
                        if (pos[POS_WQC] == 1) {
                            if (pos[FOF(7, 1)] == 0 && pos[FOF(7, 2)] == 0 &&
                                pos[FOF(7, 3)] == 0 &&
                                !isThreatened(pos, 7, 3, 1) &&
                                !isThreatened(pos, 7, 4, 1)) {
                                addMove(ml, pos, FOF(7, 4), FOF(7, 2), 0,
                                        FOF(7, 0), FOF(7, 3));
                            }
                        }
                    }
                    if (col == CB) {
                        if (pos[POS_BKC] == 1) {
                            if (pos[FOF(0, 5)] == 0 && pos[FOF(0, 6)] == 0 &&
                                !isThreatened(pos, 0, 4, CB) &&
                                !isThreatened(pos, 0, 5, CB)) {
                                addMove(ml, pos, FOF(0, 4), FOF(0, 6), 0,
                                        FOF(0, 7), FOF(0, 5));
                            }
                        }
                        if (pos[POS_BQC] == 1) {
                            if (pos[FOF(0, 1)] == 0 && pos[FOF(0, 2)] == 0 &&
                                pos[FOF(0, 3)] == 0 &&
                                !isThreatened(pos, 0, 3, CB) &&
                                !isThreatened(pos, 0, 4, CB)) {
                                addMove(ml, pos, FOF(0, 4), FOF(0, 2), 0,
                                        FOF(0, 0), FOF(0, 3));
                            }
                        }
                    }
    ...
    int stratVal(char *pos, int col) {
        ...
        for (cy = 0; cy < 8; cy++) {
            for (cx = 0; cx < 8; cx++) {
                ...
                if (pos[FOF(cy + col, cx + 1)] == FIELD_WP * col)
                    sval[of] += 4;  // atc+=vf;
                if (pos[FOF(cy + col, cx - 1)] == FIELD_WP * col)
                    sval[of] += 4;  // atc+=vf;
                for (d = 0; d < sizeof(no) / sizeof(no[0]); d++) {
                ...
                for (d = 0; d < sizeof(bo) / sizeof(bo[0]); d++) {
                    ...
                    while (val) {
                        of0 += bo[d];
                        if (pos[of0] == FIELD_BB * col)
                            sval[of] -= 3;  // atc-=vf;
                        if (pos[of0] == FIELD_BQ * col)
                            sval[of] -= 2;  // atc-=vf;
                        if (pos[of0] == FIELD_WB * col)
                            sval[of] += 3;  // atc+=vf;
                        if (pos[of0] == FIELD_WQ * col)
                            sval[of] += 2;  // atc+=vf;
                        if (pos[of0] != FIELD_EMPTY)
                            val = false;
                    }
                ...
                for (d = 0; d < sizeof(ro) / sizeof(ro[0]); d++) {
                    ...
                    while (val) {
                        of0 += ro[d];
                        if (pos[of0] == FIELD_BR * col)
                            sval[of] -= 3;  // atc-=vf;
                        if (pos[of0] == FIELD_BQ * col)
                            sval[of] -= 2;  // atc+=vf;
                        if (pos[of0] == FIELD_WR * col)
                            sval[of] += 3;  // atc-=vf;
                        if (pos[of0] == FIELD_WQ * col)
                            sval[of] += 2;  // atc+=vf;
                        if (pos[of0] != FIELD_EMPTY)
        ...
        int znorm = 4;
        atc += (sval[FOF(3, 3)] + sval[FOF(3, 4)] + sval[FOF(4, 3)] +
                sval[FOF(4, 4)]) *
               znorm;
        atc += (sval[FOF(3, 2)] + sval[FOF(3, 5)] + sval[FOF(4, 2)] +
                sval[FOF(4, 5)]) *
               znorm;
    ...
    wstring movSc(char *pos) {
        ...
        wstring res = strmov + L" (" + sig + std::to_wstring(fc1) + L"." +
                      std::to_wstring(fc2) + L")";
        return res;
    }
    ...
    int alphabeta(char *pos, int a, int b, int col, bool bMax, int depth,
                  int curdepth, int *pcurnodes, time_t maxtime,
                  wstring &movstack, char **pnpos, vector<char *> *pmlo) {
        int bsc, sc, bv, v;
        char *bp = nullptr;
        int nr, silnr;
        bool bTo = false;
        ...
        stack0 = movstack;
        bool notCheck = true;
        if (col == CW && pos[POS_WK_CHK])
            notCheck = false;
        if (col == CB && pos[POS_BK_CHK])
            notCheck = false;
        ...
        if (bMax) {
            ...
            for (auto pi : ml) {
                ...
                if (!cacheRead(pi, &v, depth - curdepth)) {
                    v = alphabeta(pi, a, b, CB, false, depth - 1, curdepth + 1,
                                  pcurnodes, maxtime, movstack, nullptr,
                                  nullptr);
            ...
            for (auto pi : ml) {
                ...
                if (!cacheRead(pi, &v, depth - curdepth + 1)) {
                    v = alphabeta(pi, a, b, CW, true, depth - 1, curdepth + 1,
                                  pcurnodes, maxtime, movstack, nullptr,
                                  nullptr);
                    if (v != SC_TIMEOUT_INVALID)
    ...
    int makeMove(char *pos, int col, std::map<string, int> &heuristic,
                 int *pcurnodes, char **pnpos) {
        int sc = 0;
        ...
        for (depthi = 4; depthi <= depth; depthi++) {
            ...
            int b = SC_MAX;
            wstring buf = L"";
            sc = alphabeta(pos, a, b, col, true, depthi, 0, pcurnodes, maxtime,
                           buf, pnpos, &ml);
...
int main(int argc, char *arv[]) {
    ...
    Chess c(1000000);  // no. of cache entries
    ...
    while (bOk) {
        ...
        if (npos != nullptr) {
            ...
            c.printPos(pos, -1);
            move = c.movSc(pos);
            wcout << move << L" Nodes: " << curnodes << L", Nodes/sec: " << nps
                  << L", time:" << dt << L"s" << endl;
            wcout << L" cache-hits:" << c.cachehit << L", cachemiss:"
                  << c.cachemiss << L", cacheconfl:" << c.cacheclash << endl;
            col = col * (-1);
